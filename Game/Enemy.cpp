...
void Enemy::Initialize(Model* model, Camera* camera, Vector3& position) {
	...
	worldTransform_.SetTransform(position);// 初期位置をオリジンにしておく
...
Vector3 Enemy::GetWorldPosition() { return {worldTransform_.translation_.x, worldTransform_.translation_.y, worldTransform_.translation_.z}; }
