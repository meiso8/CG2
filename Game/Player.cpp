...
void Player::CheckCollisionTop(CollisionMapInfo& info) {
	...
	for (uint32_t i = 0; i < positionsNew.size(); ++i) {
		positionsNew[i] = CornerPosition(worldTransform_.translation_ + info.moveVol, static_cast<Corner>(i));
	}
	...
	if (hit) {
		// めり込みを排除する方向に移動量を設定
		indexSet = mapChipField_->GetMapChipIndexSetByPosition(worldTransform_.translation_ + info.moveVol + Vector3(0, kHeight / 2.0f, 0));
		// めり込み先ブロックの範囲矩形
		MapChipField::Rect rect = mapChipField_->GetRectByIndex(indexSet.xIndex, indexSet.yIndex);
		info.moveVol.y = std::max(0.0f, rect.bottom - worldTransform_.translation_.y - kHeight / 2.0f - kBlank);
		// 天井に当たったことを記録する
...
void Player::CheckCollisionBottom(CollisionMapInfo& info) {
	...
	if (hit) {
		...
		if (indexSetNow.yIndex != indexSet.yIndex) {
			...
			// めり込み先ブロックの範囲矩形
			MapChipField::Rect rect = mapChipField_->GetRectByIndex(indexSet.xIndex, indexSet.yIndex);
			info.moveVol.y = std::min(0.0f, rect.top - worldTransform_.translation_.y + kHeight / 2.0f + kBlank);
			// 地面に当たったことを記録する
			info.isLanding = true;
		}
	}
...
void Player::CheckCollisionRight(CollisionMapInfo& info) {
	...
	if (hit) {
		...
		// めり込み先ブロックの範囲矩形
		MapChipField::Rect rect = mapChipField_->GetRectByIndex(indexSet.xIndex, indexSet.yIndex);
		info.moveVol.x = std::max(0.0f, rect.left - worldTransform_.translation_.x - kWidth / 2.0f - kBlank);
		// 壁に当たったことを記録する
		info.isWallHit = true;
...
void Player::CheckCollisionLeft(CollisionMapInfo& info) {
	...
	if (hit) {
		...
		// めり込み先ブロックの範囲矩形
		MapChipField::Rect rect = mapChipField_->GetRectByIndex(indexSet.xIndex, indexSet.yIndex);
		info.moveVol.x = std::min(0.0f, rect.right - worldTransform_.translation_.x + kWidth / 2.0f + kBlank);
		// 壁に当たったことを記録する
		info.isWallHit = true;
...
void Player::ApplyResultAndMove(const CollisionMapInfo& info) {
	// 移動
	worldTransform_.translation_ += info.moveVol;
};
...
Vector3 Player::GetWorldPosition() { return {worldTransform_.translation_.x, worldTransform_.translation_.y, worldTransform_.translation_.z}; }
...
void Player::Update() {
	...
	ImGui::DragFloat3("rotation_", &worldTransform_.rotation_.x);
	ImGui::DragFloat3("translation_", &worldTransform_.translation_.x);
	...
	if (turnTimer_ > 0.0f) {
		...
		// 角度を設定
		worldTransform_.rotation_.y = EaseInOutCubic(destinationRotationY);
	}
